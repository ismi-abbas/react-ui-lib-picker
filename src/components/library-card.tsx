...
export default function LibraryCard({
	...
	return (
		<Card
			key={library.id}
			className="group flex min-h-[280px] w-auto flex-col items-start justify-between rounded-md border border-slate-200 bg-white text-slate-900 transition-all duration-200 dark:border-slate-800 dark:bg-slate-700 dark:text-slate-100"
		>
						...
						<div className="flex gap-2">
							{library.stars !== undefined && (
								<TooltipProvider delayDuration={0}>
									<Tooltip>
										<TooltipTrigger>
											<div className="flex w-fit items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-slate-900 dark:bg-slate-800 dark:text-slate-100">
										...
										<TooltipContent className="rounded bg-slate-100 px-3 py-1 text-slate-900 dark:bg-slate-800 dark:text-slate-100">
											<p>{library.stars} stars</p>
										</TooltipContent>
									</Tooltip>
								</TooltipProvider>
							)}
							{library.totalDownloads !== undefined && (
								<TooltipProvider delayDuration={0}>
									<Tooltip>
										...
										<TooltipContent className="rounded bg-slate-100 px-3 py-1 text-slate-900 dark:bg-slate-800 dark:text-slate-100">
											<p>{library.totalDownloads} downloads</p>
										</TooltipContent>
									</Tooltip>
								</TooltipProvider>
							)}
			...
			<CardFooter className="flex w-full flex-col gap-4 pt-4 text-slate-600 dark:border-slate-600 dark:text-slate-400">
